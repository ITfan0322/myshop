		...
		$
				.ajax({
					...
					data : {
						id:id,
						status:status,
						ghid:getCookie("types")=="管理员"?0:getCookie("id")
					},
					dataType : "json",
					success : function(data) {
						console.log(data)
						...
						var pstate = ''
							for (var i = 0; i < data.length; i++) {
								var str = "<tr id='sela'>"
									...
									+ data[i].number
									+ "</td>"
									+ "<td><image style='width:80px;height:60px;' src='"
									+ data[i].yyzz
									+ "'/></td>"
									+ "<td><image style='width:80px;height:60px;' src='"
									+ data[i].sqzs
									...
									+ "</td>"
									+ "<td>"
									+ "<div class='button-group'> <a class='button border-blue' href='javascript:void(0)' onclick='del("
									+ data[i].id
									+ ",\""+data[i].phone+ "\",\""+data[i].number+"\")'>"
									+ "<span class='icon-edit'></span> <span id='del"+data[i].id+"'>"+(data[i].status==1?"通过审核":"已通过")+"</span></a> </div>"
	...
	<form method="post" action="" id="listform">
		<div class="panel admin-panel">
			...
			<table class="table table-hover text-center">
				...
				<tr>
					<td colspan="8" ><span class="pagelist" id="pagelist"></span>
					<span>跳转到
					<input type="text" class="gopage" style="width:50px;"></input>
					</span>
					页 <input type="button" value = "GO" onclick="go()">
					</td>
				</tr>
			</table>
		</div>
